...
$(function () {
	...
	if(jsondata){
		...
		$("#prop_country").val(upprop.country);
		$("#prop_city").val(upprop.city);
		$("#property_id").val(upprop.id);
		$("#prop_type").val(upprop.unittype);
		
		if(upprop.unittype == "Building"){
			$("input:radio[name=prop_type1]:nth(0)").checked = true;
			
			$("#flat_no").val(upprop.builder_number);
			$("#flat_name").val(upprop.name);
			$("#flat_addr_line1").val(upprop.builder_address);
			$("#flat_addr_line2").val(upprop.builder_address1);
		...
		if(upprop.unittype == "Villa"){
			//$("#prop_type")[1].checked = true;
	
			$("#villa_no").val(upprop.no);
			$("#villa_name").val(upprop.name);
			$("#villa_addr_line1").val(upprop.address);
			$("#villa_addr_line2").val(upprop.address1);
			$("#no_rooms").val(upprop.rooms);
			
			if(upprop.yard == 1){
				$("#villa_yard")[0].checked = true;
			}else{
				$("#villa_yard")[1].checked = true;
			}
			
			if(upprop.swimming_pool == 1){
				$("#villa_swim")[0].checked = true;
			}else{
				$("#villa_swim")[1].checked = true;
			}
			
			if(upprop.parking == 1){
				$("#villa_parking")[0].checked = true;
			}else{
				$("#villa_parking")[1].checked = true;
			}
			
			$("#villa_rent_amt").val(upprop.income);
			
			if(upprop.occupied == "YES"){
				$("#villa_occupied")[0].checked = true;
			}else{
				$("#villa_occupied")[1].checked = true;
			}
		}
		if(upprop.unittype == "Warehouse"){
			$('input:radio[name=prop_type1]:nth(2)').attr("checked",true);
			
			$("#villa").addClass("div_disable");
			$("#villa").removeClass("div_enable");	
			
			$("#wh_name").val(upprop.name);
			$("#wh_no").val(upprop.number);
			$("#wh_addr_line1").val(upprop.address);
			$("#wh_addr_line2").val(upprop.address1);
			if(upprop.measurement_is_sq_ft){
				$("#wh_measure").val(upprop.measurement_is_sq_ft_value);
				$("#wh_size").val("Sq.Ft");
			}else{
				if(upprop.measurement_is_sq_meter){
					$("#wh_measure").val(upprop.measurement_is_sq_value);
					$("#wh_size").val("Sq.Mtr");
				}
			}
			$("#wh_rent").val(upprop.income);
			$("#wh_rent_type").val(upprop.recurring);
								...
								<div class="bs-example bs-example-tabs" role="tabpanel"
                                    ...
                                    <div id="myTabContent" class="tab-content">
                                        <div role="tabpanel" class="tab-pane fade in active"
                                         ...
                                         <h2 class="h2.-bootstrap-heading group-mail">Update Property</h2>
                    				<div class="row">
                                            <div class="col-xs-6 col-md-4 group-mail">
                                            <input type="hidden" id="property_id" name="property_id" value=""/>
                                            <input type="hidden" id="prop_type" name="prop_type" value=""/>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="radio block"><input id="prop_type1" name="prop_type1" type="radio" value="Building" disabled/><label>Building</label></div>
                                        <div class="radio block"><input id="prop_type1" name="prop_type1" type="radio" value="Villa" disabled checked="true"/><label>Villa</label></div>
                                        <div class="radio block"><input id="prop_type1" name="prop_type1" type="radio" value="Warehouse" disabled /><label> Warehouse</label></div>
                                    ...
                                    <div id="warehouse" class="div_disable">
                                        ...
                                        <div class="row">
                                            <div class="col-md-3 group-mail">
                                            <label for="wh_rent">Rent Value (AED)</label>
